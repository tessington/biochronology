...
real.year.list.2.use <- min(thedata$Year): max(thedata$Year)
years.2.use <- which(real.year.list %in% real.year.list.2.use)

# calculate polygones
bottom.poly<-fitted.qs[years.2.use,1]
top.poly<-fitted.qs[years.2.use,3]
span=.07
degree=2
...
if (smooth) {
  ...
  bottom.poly.smooth<-loess.smooth(real.year.list[years.2.use], bottom.poly, span = span, degree = degree,
                                   family = family, evaluation = length(real.year.list) * 4)
  top.poly.smooth<-loess.smooth(real.year.list[years.2.use], top.poly, span = span, degree = degree,
                                family = family, evaluation = length(real.year.list)*4)
...
plotfilename <- paste("graphics/",spc,"2.pdf", sep = "")
plot.matrix <- c(1,1,2,2,2,2,2)
...
# make fitted q plot 
col.2.plot<-col2rgb(color.list[1], alpha = 0.2)
col.2.shade<-add.alpha(color.list[1],0.5)
...
lines(real.year.list[years.2.use],fitted.qs[years.2.use,2],lwd=2,col = color.list[1])
...
col.2.plot<-col2rgb(color.list[2], alpha = 0.2)
col.2.shade<-add.alpha(color.list[2],0.5)
col.2.plot <- rgb(col.2.plot[1]/255, col.2.plot[2]/255, col.2.plot[3]/255, alpha = 0.2)
polygon(x.poly,y.poly,col=col.2.plot,border=col.2.plot)
lines(real.year.list,fitted.betas[,2],lwd=2,col = color.list[2])
